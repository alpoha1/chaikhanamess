...
LEGACY_GENERAL_CHAT_NAME = "Р§Р°Р№С…Р°РЅР°"
GENERAL_CHAT_AVATAR = (
...
def init_db():
    ...
    legacy_chat = db.execute(
        ...
        (LEGACY_GENERAL_CHAT_NAME,),
    ).fetchone()
    if legacy_chat:
        db.execute(
            "UPDATE chats SET title = ? WHERE id = ?",
            (GENERAL_CHAT_NAME, legacy_chat["id"]),
        )
        db.commit()
...
def ensure_chat_membership(db, chat_id, user_id):
    db.execute(
        """
        INSERT OR IGNORE INTO chat_members (chat_id, user_id, muted, mute_reason)
        VALUES (?, ?, 0, '')
...
def add_group_member():
    ...
    if not g.current_user["is_admin"]:
        return jsonify({"error": "Только админ может добавлять участников"}), 403
    ...
    ensure_chat_membership(db, chat_id, user_id)
    chat = db.execute("SELECT * FROM chats WHERE id = ?", (chat_id,)).fetchone()
    return jsonify({"chat": serialize_chat(db, chat, g.current_user["id"])})
...
def upload(chat_id):
    ...
    requested_name = (file.filename or "").strip()
    original_name = secure_filename(requested_name)
    if not original_name:
        suffix = Path(requested_name).suffix or ""
        original_name = f"file-{uuid.uuid4().hex}{suffix}"
    ...
    mime_type = request.form.get("mime_type") or file.mimetype or "application/octet-stream"
